...
proc esm data=Work.preProcessedData back=0 lead=5 plot=(corr errors 
		modelforecasts) 
		out=prod.eprod_dt_output 
		outest=prod.eprod_dt_est 
		outstat=prod.eprod_dt_fit 
		outfor=prod.eprod_dt_forecast 
		outsum=prod.eprod_dt_summary;
	by country;
	id year interval=day;
	forecast eproduct / alpha=0.05 model=damptrend transform=none;
run;
